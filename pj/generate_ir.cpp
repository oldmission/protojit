...
mlir::Value GeneratePass::generateVectorCopyLoop(
  ...
  auto src_elem =
      _.create<VectorIndexOp>(loc, src.getType().cast<VectorType>()->elem, src,
                              src_start, idx, src_region, outline_buf);
  auto dst_elem =
      _.create<VectorIndexOp>(loc, dst.getType().cast<VectorType>()->elem, dst,
                              dst_start, idx, dst_region, outline_buf);
  ...
  // If dst is outline
  {
    ...
    _.create<scf::YieldOp>(
        loc, generateVectorCopyLoop(
                 ...
                 VectorRegion::OutlineBuf, outline_count, buf, result_buf));
  }
...
mlir::Value GeneratePass::transcodeOutlineVector(mlir::Location loc,
  ...
  {  // dst is inline
    ...
    _.create<scf::YieldOp>(
        loc, generateVectorCopyLoop(loc, _, src, zero, VectorRegion::OutlineRef,
  ...
  {  // dst is outline
    ...
    if (src_type->ppl_count <= dst_type->ppl_count) {
      ...
      result_buf = generateVectorCopyLoop(
          loc, _, src, zero, VectorRegion::OutlineRef, dst, src_ppl_count,
          VectorRegion::Partial, dst_ppl_leftover, buf, result_buf);
      _.create<scf::YieldOp>(
          loc, generateVectorCopyLoop(loc, _, src, dst_ppl_leftover,
                                      VectorRegion::OutlineRef, dst, zero,
                                      VectorRegion::OutlineBuf, outline_count,
                                      buf, result_buf));
    } else {
      ...
      result_buf = generateVectorCopyLoop(
          ...
          VectorRegion::OutlineBuf, src_ppl_leftover, buf, result_buf);
      _.create<scf::YieldOp>(
          loc, generateVectorCopyLoop(
                   loc, _, src, zero, VectorRegion::OutlineRef, dst,
                   src_ppl_leftover, VectorRegion::OutlineBuf,
