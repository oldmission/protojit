...
void VectorHoisting::hoistVectors(OutlineVariantType var) {
  ...
  for (uintptr_t i = 0; i < terms.size(); ++i) {
    ...
    auto set_attribute = [&split](Term& t,
      ...
      attributes.storage().push_back(TermAttribute{
          .value = {TermAttribute::VectorSplit{
              ...
              .path = split->path}},
          .kind = TermAttribute::kVectorSplit,
      });
    ...
    auto data = OutlineVariant(var);
    data.terms = term_conv.get();
    var.setTypeData(data);
    break;
  }
}

bool VectorHoisting::findOutlineAndHoist(mlir::Type type) {
...
bool VectorHoisting::run(Protocol& proto) {
  return findOutlineAndHoist(proto.head);
}
